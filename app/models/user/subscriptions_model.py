...
class Subscriptions(Base):
    ...
    count_free_orders = Column(Integer, nullable=True, comment="Кол-во шмоток в подписке")
    count_orders_available_by_subscription = Column(Integer, nullable=True, comment="Кол-во заказов доступных по подписке")
    count_orders_closed_by_subscription = Column(Integer, nullable=True, comment="Заказы по подписке")
    ...
    subscription_type_uuid = mapped_column(UUID(as_uuid=True), ForeignKey("subscription_types.uuid"), nullable=False, comment="Тип подписки")