...
class OrderRepository:
    ...
    def get_order_by_id(self, uuid: UUID4) -> Optional[OrderRead]:
        return self.db_session.query(Order).filter(Order.id == uuid).first()

    def list_orders_by_user_id(
        self, user_id: UUID4, skip: int = 0, limit: int = 100
    ) -> list[OrderRead]:
        return (
    ...
    def update_order(self, uuid: UUID4, order_data: OrderUpdate) -> Optional[OrderRead]:
        order = self.db_session.query(Order).filter(Order.id == uuid).first()
        if order:
