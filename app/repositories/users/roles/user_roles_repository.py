...
class UserRoleRepository(CachingMixin, BulkActionsMixin, SearchMixin):
    ...
    def get_role_by_id(self, role_id: UUID) -> Optional[Roles]:
        """Retrieves a role by ID."""

        role = self.db_session.execute(
    ...
    def get_all_roles(self) -> list[Roles]:
        ...
        return self.db_session.execute(select(Roles).where(Roles.deleted_at.is_(None)))

    def create_role(self, role_data: RoleCreate) -> Roles:
        """Creates a new role."""

        new_role = Roles(**role_data.model_dump())
        self.db_session.add(new_role)
        self.db_session.commit()
        self.db_session.refresh(new_role)
        return new_role

    def update_role(self, role_id: UUID, role_data: RoleUpdate) -> Roles:
        """Updates a role."""
    ...
    def assign_role_to_user(self, user_id: UUID, role_id: UUID) -> None:
        ...
        existing_role = self.db_session.execute(
    ...
    def remove_role_from_user(self, user_id: UUID, role_id: UUID) -> None:
        ...
        role_to_delete = self.db_session.execute(
            select(UserRoles).where(
                UserRoles.user_id == user_id, UserRoles.role_id == role_id
            )
        )
        ...
        self.db_session.delete(role_to_delete)
        self.db_session.commit()

    def get_user_roles(self, user_id: UUID) -> list[RoleRead]:
