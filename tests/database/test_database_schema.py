...
def test_database_schema_consistency(model_name):
    ...
    model_columns = {col.name for col in getattr(__import__('app.models.' + model_name.replace('_', '.'), fromlist=['']), model_name.title()).__table__.columns}
    assert db_columns == model_columns, f"{model_name.title()} model fields do not match with DB schema."