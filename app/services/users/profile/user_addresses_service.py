...
class UserAddressesService:
    ...
    async def get_addresses_by_user_id(self, user_id: UUID4) -> list[UserAddressRead]:
        """
    ...
    async def create_user_address(
        ...
        address = await self.user_address_repository.add_user_address(
    ...
    async def update_user_address(
        ...
        address = await self.user_address_repository.update_user_address(
    ...
    async def set_default_address(
        ...
        address_to_set_default = await self.get_address_by_id(address_id)
        ...
        existing_default_address = (
            await self.user_address_repository.get_address_by_user_id_and_type(
                user_id, address_type
            )
        )

        async with self.user_address_repository.db_session as session:
            ...
            if existing_default_address:
                ...
                await existing_default_address.update(session)

            # 4. Update the selected address to be the default.
            address_to_set_default.address_type = address_type
            await address_to_set_default.update(session)

        return UserAddressRead.model_validate(address_to_set_default)

    async def validate_address(self, address_data: UserAddressCreate) -> bool:
        """
    ...
    async def get_preferred_address(self, user_id: UUID4) -> Optional[UserAddressRead]:
        """
