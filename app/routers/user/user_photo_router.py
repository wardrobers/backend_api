...
def add_user_photo(user_id: UUID4, request: Request, file: UploadFile = File(...)):
    """
    ...
    photo_data = {"user_id": user_id, "storage_url": storage_url}
    photo = UserPhotoRepository(db).add_photo(photo_data)
    return photo


@router.get("/{user_id}/photos/", response_model=list[UserPhotoRead])
def get_user_photos(user_id: UUID4, request: Request):
    """
    Retrieve all photos for a given user.
    """
    db: Session = request.state.db
    photos = UserPhotoRepository(db).get_photos_by_user_id(user_id)
    return photos
...
def update_user_photo(
    ...
    updated_photo = UserPhotoRepository(db).update_photo(
        photo_id, photo_update.dict(exclude_unset=True)
    )
...
@router.delete("/{user_id}/photos", status_code=status.HTTP_204_NO_CONTENT)
def delete_user_photo(photo_id: UUID4, request: Request):
    """
    Delete a user photo.
    """
    db: Session = request.state.db
    UserPhotoRepository(db).delete_photo(photo_id)
