...
def test_user(db_session: Session) -> Users:
    return create_random_user(db_session)


@pytest.fixture
def test_user_info(db_session: Session, test_user: Users) -> UserInfo:
    return create_random_user_info(db_session, test_user)


@pytest.fixture
def test_role(db_session: Session) -> Roles:
    return create_random_role(db_session)


@pytest.fixture
def test_address(db_session: Session, test_user: Users) -> UserAddresses:
...
@pytest.fixture
def access_token(db_session: Session, test_user: Users):
    user_data = UserLogin(login=test_user.login, password="testpassword")
    return get_access_token(db_session, user_data)
...
@pytest.mark.asyncio
def test_login_user(test_user: Users):
    user_data = UserLogin(login=test_user.login, password="testpassword")
...
@pytest.mark.asyncio
def test_login_user_invalid_credentials(test_user: Users):
...
@pytest.mark.asyncio
def test_get_current_user(test_user: Users, access_token: str):
    response = client.get(
...
@pytest.mark.asyncio
def test_update_current_user(db_session: Session, test_user: Users, access_token: str):
...
@pytest.mark.asyncio
def test_update_current_user_invalid_data(access_token: str):
...
@pytest.mark.asyncio
def test_delete_current_user(db_session: Session, test_user: Users, access_token: str):
    response = client.delete(
        "/users/me", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 204
    # Verify the user is soft-deleted
    deleted_user = Users.get_by_id(db_session, test_user.id)
    assert deleted_user.deleted_at is not None


# --- User Info Tests ---
@pytest.mark.asyncio
def test_update_current_user_info(access_token: str):
...
@pytest.mark.asyncio
def test_update_current_user_info_invalid_data(access_token: str):
...
# --- Role Tests ---
@pytest.mark.asyncio
def test_get_all_roles(access_token: str):
    response = client.get(
        "/users/roles/", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 200
...
@pytest.mark.asyncio
def test_create_role(access_token: str):
...
@pytest.mark.asyncio
def test_get_role_by_id(test_role: Roles, access_token: str):
    response = client.get(
...
@pytest.mark.asyncio
def test_update_role(test_role: Roles, access_token: str):
...
@pytest.mark.asyncio
def test_delete_role(test_role: Roles, access_token: str):
    response = client.delete(
        f"/users/roles/{test_role.id}",
        headers={"Authorization": f"Bearer {access_token}"},
    )
...
# --- User Address Tests ---
@pytest.mark.asyncio
def test_add_user_address(access_token: str):
    address_data = UserAddressCreate(
...
@pytest.mark.asyncio
def test_get_all_user_addresses(access_token: str):
    response = client.get(
        "/users/addresses/", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 200
...
@pytest.mark.asyncio
def test_update_user_address(test_address: UserAddresses, access_token: str):
...
@pytest.mark.asyncio
def test_delete_user_address(test_address: UserAddresses, access_token: str):
    response = client.delete(
        f"/users/addresses/{test_address.id}",
        headers={"Authorization": f"Bearer {access_token}"},
    )
