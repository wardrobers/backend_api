...
from app.repositories.users import UserInfoRepository, UsersRepository, UserAddressRepository, UserPhotosRepository, UserRoleRepository
from app.services.users import UsersService
from app.schemas.users import UsersCreate, UsersUpdate, UserInfoCreate, UserInfoUpdate, UpdateContext
...
@pytest.mark.asyncio
async def test_get_user_by_id(user_service: UsersService):
    user = await user_service.get_user_by_id(UUID("00000000-0000-0000-0000-000000000001"))
    assert user.id == UUID("00000000-0000-0000-0000-000000000001")
    assert user.login == "testuser"
    user_service.users_repository.get_user_by_id.assert_called_once_with(UUID("00000000-0000-0000-0000-000000000001"))

@pytest.mark.asyncio
async def test_get_user_by_id_not_found(user_service: UsersService):
    user_service.users_repository.get_user_by_id.return_value = None
    with pytest.raises(HTTPException) as exc_info:
        await user_service.get_user_by_id(UUID("00000000-0000-0000-0000-000000000002"))
    assert exc_info.value.status_code == 404
    assert exc_info.value.detail == "User not found"

@pytest.mark.asyncio
async def test_get_user_by_login(user_service: UsersService):
    user = await user_service.get_user_by_login("testuser")
    assert user.id == UUID("00000000-0000-0000-0000-000000000001")
    assert user.login == "testuser"
    user_service.users_repository.get_user_by_login.assert_called_once_with("testuser")

@pytest.mark.asyncio
async def test_get_user_by_login_not_found(user_service: UsersService):
    user_service.users_repository.get_user_by_login.return_value = None
    with pytest.raises(HTTPException) as exc_info:
        await user_service.get_user_by_login("nonexistentuser")
    assert exc_info.value.status_code == 404
    assert exc_info.value.detail == "User not found"

@pytest.mark.asyncio
async def test_get_all_users(user_service: UsersService):
...
@pytest.mark.asyncio
async def test_create_user(user_service: UsersService, db_session: AsyncSession):
    ...
    user_service.user_info_repository.create_user_info.assert_called_once_with(
        created_user.id, UserInfoCreate(email=user_data.login, first_name='', last_name='')
    )

@pytest.mark.asyncio
async def test_create_user_passwords_dont_match(user_service: UsersService):
    user_data = UsersCreate(login="newuser", password="securepassword", password_confirmation="differentpassword")
    with pytest.raises(HTTPException) as exc_info:
...
@pytest.mark.asyncio
async def test_create_user_login_already_exists(user_service: UsersService):
    user_data = UsersCreate(login="testuser", password="securepassword", password_confirmation="securepassword")
    with pytest.raises(HTTPException) as exc_info:
        await user_service.create_user(user_data)
    assert exc_info.value.status_code == 400
...
@pytest.mark.asyncio
async def test_update_user(user_service: UsersService):
    user_id = UUID("00000000-0000-0000-0000-000000000001")
    user_update = UsersUpdate(login="updateduser", is_notificated=True)
    updated_user = await user_service.update_user(user_id, user_update, current_user=Users(id=user_id))
    assert updated_user.id == user_id
    assert updated_user.login == "updateduser"
    assert updated_user.is_notificated == True
    user_service.users_repository.update_user.assert_called_once_with(user_id, user_update)

@pytest.mark.asyncio
async def test_update_user_not_found(user_service: UsersService):
    user_id = UUID("00000000-0000-0000-0000-000000000002")
    user_update = UsersUpdate(login="updateduser")
    user_service.users_repository.get_user_by_id.return_value = None 
    with pytest.raises(HTTPException) as exc_info:
        await user_service.update_user(user_id, user_update, current_user=Users(id=user_id))
    assert exc_info.value.status_code == 404
    assert exc_info.value.detail == "User not found"

@pytest.mark.asyncio
async def test_delete_user(user_service: UsersService):
    user_id = UUID("00000000-0000-0000-0000-000000000001")
    await user_service.delete_user(user_id, current_user=Users(id=user_id, is_superuser=True)) # Assuming is_superuser for authorization
    user_service.user_address_repository.get_addresses_by_user_id.assert_called_once_with(user_id)
...
@pytest.mark.asyncio
async def test_delete_user_not_found(user_service: UsersService):
    user_id = UUID("00000000-0000-0000-0000-000000000002")
    user_service.users_repository.get_user_by_id.return_value = None
    with pytest.raises(HTTPException) as exc_info:
        await user_service.delete_user(user_id, current_user=Users(id=user_id))
    assert exc_info.value.status_code == 404
    assert exc_info.value.detail == "User not found"

@pytest.mark.asyncio
async def test_update_user_info(user_service: UsersService):
    user_id = UUID("00000000-0000-0000-0000-000000000001")
    user_info_update = UserInfoUpdate(first_name="Updated", last_name="User", email="updated@example.com", lender=True)
    updated_user_info = await user_service.update_user_info(user_id, user_info_update, UpdateContext.FULL_PROFILE)
    assert updated_user_info.first_name == "Updated"
    assert updated_user_info.lender == True 
    user_service.user_info_repository.update_user_info.assert_called_once_with(user_id, user_info_update)

@pytest.mark.asyncio
async def test_update_user_info_not_found(user_service: UsersService):
    user_id = UUID("00000000-0000-0000-0000-000000000002")
    ...
    with pytest.raises(HTTPException) as exc_info:
        await user_service.update_user_info(user_id, user_info_update, UpdateContext.FULL_PROFILE)
    assert exc_info.value.status_code == 404
    assert exc_info.value.detail == "User info not found"

@pytest.mark.asyncio
async def test_toggle_notifications(user_service: UsersService):
    user_id = UUID("00000000-0000-0000-0000-000000000001")
    await user_service.toggle_notifications(user_id, True, current_user=Users(id=user_id))
    user_service.users_repository.toggle_notifications.assert_called_once_with(user_id, True)

@pytest.mark.asyncio
async def test_toggle_notifications_user_not_found(user_service: UsersService):
    user_id = UUID("00000000-0000-0000-0000-000000000002")
    user_service.users_repository.get_user_by_id.return_value = None
    with pytest.raises(HTTPException) as exc_info:
        await user_service.toggle_notifications(user_id, True, current_user=Users(id=user_id))
